...
public class B_JpaDaoTest {
   ...
   * TESTS
    ...
    @Test
    public void g_testerLireListe() throws MyDBException {
        System.out.println("*** " + SystemLib.getCurrentMethod() + " ...");
        ...
        @Test
    public void h_testerRechercher() throws MyDBException {
            System.out.println("*** "+ SystemLib.getCurrentMethod() + " ...");
        List<Personne> list = persWrk.lireListe();
        boolean ok = false;
        if (list != null && !list.isEmpty()) {
            Personne p = persWrk.rechercher("nom", list.get(0).getNom());
            if (p != null && p.getNom().equals(list.get(0).getNom())) {
                ok = true;
                System.out.println("Le nom de la personne dans la liste " + list.get(0).getNom() + " est égale au nom rechercher" + p.getNom());
            }
        }
        assertTrue(ok);
    }
